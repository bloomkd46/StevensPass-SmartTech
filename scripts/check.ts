...
async function checkBranch() {
  if (process.env['GITHUB_BASE_REF'] === 'main' && process.env['GITHUB_HEAD_REF'] !== 'development') {
    log.error('Only the development branch may merge into the main branch!');
    summary.push(`- [ ] Branch Check: Only the development branch may merge into the main branch! (\`${process.env['GITHUB_BASE_REF']} === 'main' && ${process.env['GITHUB_HEAD_REF']} !== 'development')\``);
    success = false;
  } else {
